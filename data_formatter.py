...
yacht = pd.read_csv("./data/origin_data/yacht_hydrodynamics.data", sep=" ", header=None)
yacht.columns=["Longitudinal", "Prismatic", "Length-displacement", "Beam-draught", "Length-beam", "Froude", "label"]
for col in yacht.columns:
    yacht.loc[:, col] = (yacht[col] - yacht[col].mean()) / yacht[col].std(ddof=0)
yacht.to_csv("./data/yacht.data",sep=",", index=None)